...
class SurveyListApplication extends Component {
    ...
    handleCheckChange(event, id) {
        ...
        let selectedItems = Object.assign([], this.props.surveyListApp.selectedItems);
    ...
    handleChangeSelectAll() {
        if (this.props.surveyListApp.loading) {
            if (this.props.surveyListApp.selectedItems.length >= this.props.surveyListApp.surveyItems.length) {
                this.props.selectedSurveyItemsChange([]);
            } else {
                this.props.selectedSurveyItemsChange(this.props.surveyListApp.surveyItems.map(x => x.id));
            }
        }
    }
                ...
                <Row className="app-row survey-app">
                    <Colxx xxs="12">
                                    ...
                                    <div className="btn btn-primary pl-4 pr-0 check-button">
                                        <Label
                                            ...
                                            <Input
                                                ...
                                                type="checkbox"
                                                id="checkAll"
                                                checked={loading && selectedItems.length >= surveyItems.length}
                        ...
                        <div className="mb-2">
                            ...
                            <Collapse className="d-md-block" isOpen={this.state.displayOptionsIsOpen}>
                                                ...
                                                <div className="d-flex flex-grow-1 min-width-zero">
                                                    ...
                                                    <div className="custom-control custom-checkbox pl-1 align-self-center pr-4">
                                                        <CustomInput
                                                            ...
                                                            type="checkbox"
                                                            id={`check_${item.id}`}
                                                            checked={loading && selectedItems.includes(item.id)}
                                                            onClick={event => this.handleCheckChange(event, item.id)}
                                                            label=""
                                                        />
                                                    </div>
                                                </div>
                            ...
                            <ul className="list-unstyled mb-5">
                                <NavItem className={classnames({ active: !filter })}>
                                    <NavLink to="#" onClick={e => this.addFilter('', '')}>
                                        ...
                                        <span className="float-right">{loading && allSurveyItems.length}</span>
                                    </NavLink>
                                </NavItem>
                                <NavItem
                                    ...
                                    <NavLink to="#" onClick={e => this.addFilter('status', 'ACTIVE')}>
                                        <i className="simple-icon-refresh" />
                                        <IntlMessages id="survey.active-surveys" />
                                        <span className="float-right">
                                            {loading && surveyItems.filter(x => x.status == 'ACTIVE').length}
                                        </span>
                                    </NavLink>
                                </NavItem>
                                <NavItem
                                    ...
                                    <NavLink to="#" onClick={e => this.addFilter('status', 'COMPLETED')}>
                                        <i className="simple-icon-check" />
                                        <IntlMessages id="survey.completed-surveys" />
                                        <span className="float-right">
                                            {loading && surveyItems.filter(x => x.status == 'COMPLETED').length}
                                        </span>
                                    </NavLink>
                                </NavItem>
                            </ul>
                                            ...
                                            <div onClick={e => this.addFilter('category', c)}>
                                                <div className="custom-control custom-radio">
                                                    <input
                                                        ...
                                                        className="custom-control-input"
                                                        defaultChecked={
                                                            filter && filter.column === 'category' && filter.value === c
                                                        }
                                                    />
                                ...
                                {labels.map((l, index) => {
                                    return (
                                        <p className="d-sm-inline-block mb-1" key={index}>
                                            <NavLink to="#" onClick={e => this.addFilter('label', l.label)}>
                                                <Badge
                                                    className="mb-1"
                                                    color={`${
                                                        filter && filter.column === 'label' && filter.value === l.label
                                                            ? l.color
                                                            : 'outline-' + l.color
                                                    }`}
                                                    pill>
