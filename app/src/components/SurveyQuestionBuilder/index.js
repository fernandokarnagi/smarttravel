...
export default class SurveyQuestionBuilder extends React.Component {
    ...
    typeChange(answerType) {
        if (this.state.answerType) {
            if ((this.state.answerType.id === 2 || this.state.answerType.id === 3) && answerType.id == 1) {
    ...
    updateAnswer(answerId, event) {
        var answerIndex = this.state.answers.findIndex(item => item.id === answerId);
    ...
    renderViewModeAnswers() {
        ...
        switch (this.state.answerType.id) {
            ...
            default:
                return <Input type="text" placeholder="" value={''} onChange={event => {}} />;
        }
    }
                ...
                <div className="d-flex flex-grow-1 min-width-zero">
                    <div className="card-body align-self-center d-flex flex-column flex-md-row justify-content-between min-width-zero align-items-md-center">
                        <div className="list-item-heading mb-0 truncate w-80 mb-1 mt-1">
                            <span className="heading-number d-inline-block">{this.props.order + 1}</span>
                            {this.state.title}
                        </div>
                    </div>
                    <div className="custom-control custom-checkbox pl-1 align-self-center pr-4">
                        ...
                        <Button outline color={'theme-3'} className="icon-button ml-1" onClick={this.deleteClick}>
